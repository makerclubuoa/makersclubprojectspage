...
async function fetchGhostMembers() {
  ...
  for (let page = 1; ; page += 1) {
    const response = await fetch(`${ghostUrl}/ghost/api/admin/members/?limit=100&page=${page}`, {
      headers: {
    ...
    if (!pagination || page >= pagination.pages) return members;
  }
}
...
async function fetchProfiles() {
  ...
  for (let offset = 0; ; offset += 1000) {
    const { data, error } = await supabase
      .from("profiles")
      .select("id,email,membership_year,membership_email_confirmed_at,membership_sync_status,ghost_member_id")
      .order("id")
