...
import { fetchProject, fetchProjects, fetchAllIds, fetchMakerDisplay, categoryColor, categoryPopText } from '@/lib/projects'
import {
  container, pageWrap, holt, secHead, secHeadRow, secHint,
  btnGhost, btnGradient, btnArr, ph, phLabel,
} from '@/lib/ui'

// White comic card used for every content section on the gradient.
const SECTION = 'relative bg-white outline-solid outline-3 outline-black shadow-[6px_6px_0px_0px_#000] p-6 md:p-8 mb-10'
...
// BOM table cells
const BOM_TH = 'text-left px-3.5 py-2.5 text-[10px] font-bold tracking-[0.12em] uppercase text-ink bg-paper-2 border-b-2 border-black'
const BOM_TD = 'px-3.5 py-[11px] border-b border-black/10 align-top'
const BOM_TD_NUM = 'px-3.5 py-[11px] border-b border-black/10 align-top text-sm w-20'
const BOM_TF = 'px-3.5 py-3 text-[11px] font-bold tracking-[0.1em] uppercase border-t-2 border-black bg-paper-2 text-ink'
const BOM_TF_TOTAL = 'px-3.5 py-3 border-t-2 border-black bg-paper-2 text-ink font-bold text-[17px]'

// Sidebar table-of-contents links
const TOC_LINK = 'flex items-center gap-2.5 text-[12px] font-bold py-[5px] text-ink no-underline transition-colors duration-200 hover:text-pop-magenta'
const TOC_DOTS = ['bg-pop-blue', 'bg-pop-violet', 'bg-pop-magenta', 'bg-pop-pink', 'bg-pop-red', 'bg-pop-orange']
const SPEC_ROW = 'flex justify-between gap-3 text-xs font-medium pb-[7px] border-b border-black/10 last-of-type:border-b-0'
const SPEC_H = `${holt} text-base text-pop-violet mt-4 mb-2.5`
const SPEC_K = 'text-ink-2 font-semibold tracking-[0.06em] uppercase text-[10.5px]'
const SPEC_V = 'text-[13px] font-semibold text-right'
const FRAME = 'bg-white outline-solid outline-3 outline-black shadow-[6px_6px_0px_0px_#000] p-[18px] pt-6 relative'
const FRAME_FIG =
...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  ...
  return { title: project ? `${project.title} | Maker Club` : 'Project | Maker Club' }
}
...
export default async function ProjectPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const tocEntries = [
    ...
    ...(hasRelated  ? [{ href: '#related', label: 'More from the archive' }] : []),
  ]
        ...
        <header className="border-y-4 bg-white py-10">
          <div className={container}>
            ...
            <div className="flex flex-wrap items-center gap-2 mb-8">
              <span className={`${META_CHIP} bg-black text-white`}>{project.category ?? '—'}</span>
              <span className={META_CHIP}>Started {startedDate}</span>
              <span className={META_CHIP}>Logged {loggedDate}</span>
              {project.build_time && (
                <span className={META_CHIP}>Build {project.build_time.toUpperCase()}</span>
              )}
            </div>
            ...
            <div className="grid grid-cols-[1.05fr_1fr] gap-12 items-center max-[980px]:grid-cols-1 max-[980px]:gap-10">
              {/* Left */}
              <div>
                <h1 className={`${holt} ${titleColor} text-[clamp(26px,4vw,48px)] leading-[1.2] mt-0 mb-[18px] [text-wrap:balance]`}>{project.title}</h1>
                {project.blurb && <p className="text-lg font-semibold max-w-[52ch] mt-0 mb-6 leading-[1.5]">{project.blurb}</p>}

                {totalMakers > 0 && (
                  <div className="flex items-center gap-3 mb-5">
                    <span className="flex">
                      {Array.from({ length: totalMakers }).map((_, i) => (
                        <span key={i} className={`w-7 h-7 rounded-full border-2 border-black inline-block${i > 0 ? ' -ml-1.5' : ''}`} style={{ background: color }} />
                      ))}
                    </span>
                ...
                <div className="flex flex-wrap border-2 border-black rounded-[6px] overflow-hidden shadow-[4px_4px_0px_0px_#000]">
                  <div className="px-4 py-3.5 border-r-2 border-black flex-1 min-w-[90px] last:border-r-0 max-[480px]:flex-[1_1_calc(50%-1px)] max-[480px]:min-w-0">
                    <b className="block font-bold text-[22px] leading-none mb-[5px]">{project.likes ?? 0}</b>
                    <span className="block text-[10px] font-bold tracking-[0.1em] uppercase text-ink-2">loves</span>
                  </div>
                  <div className="px-4 py-3.5 border-r-2 border-black flex-1 min-w-[90px] last:border-r-0 max-[480px]:flex-[1_1_calc(50%-1px)] max-[480px]:min-w-0">
                    <b className="block font-bold text-[22px] leading-none mb-[5px]">{(project.tools ?? []).length}</b>
                    <span className="block text-[10px] font-bold tracking-[0.1em] uppercase text-ink-2">tools used</span>
                  </div>
                  {hasBOM && (
                    <div className="px-4 py-3.5 border-r-2 border-black flex-1 min-w-[90px] last:border-r-0 max-[480px]:flex-[1_1_calc(50%-1px)] max-[480px]:min-w-0">
                      <b className="block font-bold text-[22px] leading-none mb-[5px]">{bomItems.length}</b>
                      <span className="block text-[10px] font-bold tracking-[0.1em] uppercase text-ink-2">parts</span>
                    </div>
                  )}
                  {hasBOM && bomTotal > 0 && (
                    <div className="px-4 py-3.5 border-r-2 border-black flex-1 min-w-[90px] last:border-r-0 max-[480px]:flex-[1_1_calc(50%-1px)] max-[480px]:min-w-0">
                      <b className="block font-bold text-[22px] leading-none mb-[5px]">${bomTotal.toFixed(0)}</b>
                      <span className="block text-[10px] font-bold tracking-[0.1em] uppercase text-ink-2">est. cost</span>
                    </div>
                  )}
                </div>
              ...
              <div className="min-w-0">
                ...
                {hasStory && (
                  <section className={SECTION} id="story">
                    <div className={`${secHeadRow} mb-5`}>
                      <h3 className={`${secHead} text-pop-blue`}>The Story</h3>
                      <span className={secHint}>the full write-up</span>
                    </div>
                    <div>
                      {(project.description ?? '').split('\n\n').filter(Boolean).map((para, i) => (
                        <p key={i} className="text-[14.5px] font-medium text-ink mt-0 mb-[18px] max-w-[62ch] leading-[1.75]">{para}</p>
                      ))}
                    </div>
                  </section>
                )}
                ...
                {hasBuildLog && (
                  <section className={SECTION} id="log">
                    <div className={`${secHeadRow} mb-5`}>
                      ...
                      <span className={secHint}>{(project.build_log ?? []).length} entries</span>
                    </div>
                    <div className="relative">
                      {(project.build_log ?? []).map((entry, i) => (
                        <div key={i} className="group grid grid-cols-[110px_32px_minmax(0,1fr)] gap-[18px] py-[18px] border-t-2 border-black/10 items-start last:border-b-2 last:border-black/10 max-[720px]:grid-cols-[80px_24px_1fr]">
                          <div className="text-sm font-semibold leading-[1.3] pt-1">
                            {entry.date}
                            {entry.week_label && <small className="block text-ink-2 text-[10px] font-bold tracking-[0.1em] uppercase mt-0.5">{entry.week_label}</small>}
                          </div>
                          <div className="relative pt-2 flex justify-center before:content-[''] before:absolute before:top-[14px] before:bottom-[-100%] before:left-1/2 before:w-[2px] before:bg-black/20 group-last:before:hidden">
                            <span className={`relative z-[1] w-3 h-3 rounded-full block border-2 border-black ${entry.milestone ? 'bg-pop-magenta shadow-[2px_2px_0px_0px_#000]' : 'bg-white'}`} />
                          </div>
                          <div>
                            <h4 className="text-[17px] font-bold m-0 mb-1.5">{entry.title}</h4>
                            <p className="m-0 text-[13.5px] font-medium text-ink-2 leading-[1.6] max-w-[56ch]">{entry.body}</p>
                            {entry.tag && <span className="inline-block mt-2 px-2.5 py-0.5 rounded-full border border-black/25 bg-paper-2 text-[10px] font-semibold tracking-[0.06em]">{entry.tag}</span>}
                            {entry.image && (
                              // eslint-disable-next-line @next/next/no-img-element
                              <img src={entry.image} alt={entry.title} className="block mt-3 max-w-full max-h-[320px] border-2 border-black object-cover" loading="lazy" />
                            )}
                          </div>
                        </div>
                      ))}
                ...
                {hasGallery && (
                  <section className={SECTION} id="gallery">
                    <div className={`${secHeadRow} mb-5`}>
                      ...
                      <span className={secHint}>{(project.gallery_images ?? []).length} photos</span>
                    </div>
                    <div className="grid grid-cols-3 gap-3 max-[720px]:grid-cols-2">
                      {(project.gallery_images ?? []).map((src, i) => (
                        <div key={i} className={`relative bg-white outline-solid outline-2 outline-black p-1 overflow-visible ${i === 0 ? 'col-span-2 row-span-2 aspect-square max-[720px]:row-span-1 max-[720px]:aspect-[4/3]' : 'aspect-[4/3]'} ${i % 3 === 1 ? 'rotate-1' : i % 3 === 2 ? '-rotate-1' : ''}`}>
                          {/* eslint-disable-next-line @next/next/no-img-element */}
                            ...
                            return (
                              <tr key={i} className="[&:last-child>td]:border-b-0">
                                <td className={BOM_TD}>
                                  ...
                                  {item.desc && <div className="text-ink-2 text-[11.5px] mt-0.5">{item.desc}</div>}
                                </td>
                  ...
                  <section className={SECTION} id="retro">
                    ...
                    <div className="grid grid-cols-2 gap-3.5 max-[720px]:grid-cols-1">
                      {(project.retro_wins ?? []).length > 0 && (
                        <div className="border-2 border-black rounded-[6px] overflow-hidden bg-white">
                          <h4 className="flex items-center gap-2.5 px-3.5 py-2.5 m-0 text-[11px] font-bold tracking-[0.14em] uppercase bg-paper-2 border-b-2 border-black">
                            <span className={`${RETRO_CHIP} bg-pop-blue`}>✓</span>
                            <span>What worked</span>
                          </h4>
                          <ul className="list-none m-0 p-3.5">
                            {(project.retro_wins ?? []).map((w, i) => (
                              <li key={i} className="flex items-start gap-2.5 py-[9px] border-b border-black/10 last:border-b-0 text-[13.5px] font-medium text-ink leading-[1.55]">
                      ...
                      {(project.retro_fixes ?? []).length > 0 && (
                        <div className="border-2 border-black rounded-[6px] overflow-hidden bg-white">
                          <h4 className="flex items-center gap-2.5 px-3.5 py-2.5 m-0 text-[11px] font-bold tracking-[0.14em] uppercase bg-paper-2 border-b-2 border-black">
                            <span className={`${RETRO_CHIP} bg-pop-red`}>✕</span>
                ...
                {hasKudos && (
                  <section className={SECTION} id="kudos">
                    <div className={`${secHeadRow} mb-5`}>
                      <h3 className={`${secHead} text-pop-orange`}>Shout Outs</h3>
                      <span className={secHint}>from the club</span>
                    </div>
                    <div className="grid grid-cols-2 gap-3 max-[720px]:grid-cols-1">
                      {(project.kudos ?? []).map((k, i) => (
                        <div key={i} className="border-2 border-black rounded-[6px] p-4 bg-paper-2 flex flex-col gap-2.5">
                          <p className="m-0 text-[13.5px] font-medium text-ink leading-[1.6] before:content-['“'] before:font-bold before:text-pop-orange before:mr-0.5 after:content-['”'] after:font-bold after:text-pop-orange after:ml-0.5">{k.text}</p>
                          <div className="flex items-center gap-2.5 text-[10.5px] font-semibold tracking-[0.08em] uppercase text-ink-2 pt-2 border-t-2 border-black/10">
                            ...
                            <span><b className="text-ink font-bold tracking-[0.04em]">{k.who}</b>{k.role ? ` · ${k.role}` : ''}</span>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <CommentsSection projectId={project.id} projectTitle={project.title} projectOwnerId={project.submitted_by ?? null} />

                {/* Related */}
                {hasRelated && (
                  <section className={SECTION} id="related">
                    <div className={`${secHeadRow} mb-5`}>
                      <h3 className={`${secHead} text-pop-violet`}>More From the Archive</h3>
                      <span className={secHint}>three picks</span>
                    </div>
                    <div className="grid grid-cols-3 gap-4 max-[980px]:grid-cols-2 max-[720px]:grid-cols-1">
                      {related.map(r => (
                        <Link key={r.id} href={`/projects/${r.id}`} className="group border-2 border-black bg-white flex flex-col no-underline text-inherit [transition:transform_0.2s_ease,box-shadow_0.2s] hover:-translate-y-0.5 hover:shadow-[4px_4px_0px_0px_#000]">
                          <div
                          ...
                          <h5 className="text-[15px] font-bold mx-[14px] mt-3 mb-1">{r.title}</h5>
                          <p className="mx-[14px] mt-0 mb-[14px] text-ink-2 text-[10.5px] font-semibold tracking-[0.06em] uppercase">{r.category} · {[
                ...
                <div className="sticky top-20 flex flex-col gap-6 max-[980px]:static">
                  ...
                  <div className={FRAME}>
                    ...
                    {tocEntries.map((entry, i) => (
                      <a key={entry.href} href={entry.href} className={TOC_LINK}>
                        <span className={`w-2.5 h-2.5 rounded-full border-2 border-black shrink-0 ${TOC_DOTS[i % TOC_DOTS.length]}`} />
                        {entry.label}
                      </a>
                    ))}
                  </div>
                  ...
                  {/* Spec */}
                  <div className={FRAME}>
                    <span className={`${FRAME_FIG} bg-pop-violet`}>At a glance</span>
                    {project.category && (
                    ...
                    {(project.tools ?? []).length > 0 && (
                      <>
                        ...
                        <div className="flex flex-wrap gap-1.5 mt-2">
                          {(project.tools ?? []).map(t => (
                            <span key={t} className="px-2.5 py-0.5 rounded-full border border-black/25 bg-paper-2 text-[11px] font-semibold text-ink-2">{t}</span>
                          ))}
                        </div>
                      </>
                    )}
                    ...
                    {(project.github) && (
                      <>
                        ...
                        {project.github && (
                          <a href={project.github} className="flex justify-between py-[9px] border-b border-black/10 last:border-b-0 text-xs font-bold text-pop-blue no-underline tracking-[0.04em] transition-colors duration-200 hover:text-pop-violet" target="_blank" rel="noopener noreferrer">
                            <span>GitHub ↗</span>
                            <small className="text-ink-2 text-[10px] font-semibold tracking-[0.08em] uppercase">source</small>
                          </a>
                        )}
                      </>
                    )}
